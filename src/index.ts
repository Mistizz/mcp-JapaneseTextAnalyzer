...
class JapaneseTextAnalyzer {
  ...
  // クリップボードからのテキストの文字数を計測する処理
  private countClipboardTextImpl(text: string) {
    try {
      // 改行とスペースを除外した文字数
      ...
      return {
        content: [{ 
          type: 'text' as const, 
          text: `クリップボードのテキストの文字数: ${effectiveCharCount}文字（改行・スペース除外）`
        }]
      };
    } catch (error: any) {
      return {
  ...
  // クリップボードからのテキストの単語数を計測する処理
  private countClipboardTextWordsImpl(text: string, language: 'en' | 'ja' = 'en') {
    try {
      let wordCount = 0;
      let resultText = '';
      ...
      if (language === 'en') {
        ...
        wordCount = words.length;
        resultText = `クリップボードのテキストの単語数: ${wordCount}単語 (英語モード)`;
      } else if (language === 'ja') {
        // 日本語の場合、kuromojiを使用して形態素解析
        if (!tokenizer) {
          return {
        ...
        resultText = `クリップボードのテキストの単語数: ${wordCount}単語 (日本語モード、すべての品詞を含む)\n\n分析結果:\n${tokenDetails}\n\n有効な単語としてカウントしたもの:\n${meaningfulTokens.map((t: any) => t.surface_form).join(', ')}`;
      }
  ...
  setupTools() {
    ...
    // クリップボードのテキストの文字数を計測するツール
    this.server.tool(
      'count-clipboard-chars', 
      'クリップボードからのテキストの文字数を計測します。スペースや改行を除いた実質的な文字数をカウントします。日本語と英語の両方に対応しています。',
      { text: z.string().describe('文字数をカウントするテキスト') },
      async ({ text }) => this.countClipboardTextImpl(text)
    );

    // クリップボードのテキストの単語数を計測するツール
    this.server.tool(
      'count-clipboard-words', 
      'クリップボードからのテキストの単語数を計測します。英語ではスペースで区切られた単語をカウントし、日本語では形態素解析を使用して単語をカウントします。日本語モードでは記号や空白を除外した有意な単語のみをカウントします。',
      { 
        text: z.string().describe('単語数をカウントするテキスト'),
        language: z.enum(['en', 'ja']).default('en').describe('テキストの言語 (en: 英語, ja: 日本語)')
      },
      async ({ text, language }) => this.countClipboardTextWordsImpl(text, language)
    );
  }
